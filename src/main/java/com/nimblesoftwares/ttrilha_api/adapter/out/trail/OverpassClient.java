...
public class OverpassClient implements OverpassPort {
  ...
  public String searchTrails(double lat, double lon, int radiusKm) {
...
//        .map(element -> {
//          List<GeoPoint> points = (element.geometry() != null && !element.geometry().isEmpty())
//              ? element.geometry()
...
//                  : List.of();
//
//          return new OverpassTrailData(element.id(), element.tags(), points);
//        })
//        .toList();
  }
}